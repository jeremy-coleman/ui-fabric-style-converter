...
export class CommandBarBase extends BaseComponent<ICommandBarProps, {}> implements ICommandBar {
  ...
  private _onRenderItem = (item: ICommandBarItemProps): JSX.Element | React.ReactNode => {
    const CommandButtonType: any = this.props.buttonAs || item.commandBarButtonAs || CommandBarButton;
    ...
    if (item.iconOnly && itemText !== undefined) {
      return (        
        <TooltipHost content={itemText} {...item.tooltipHostProps}><CommandButtonType {...commandButtonProps} defaultRender={CommandBarButton} />
        </TooltipHost>
      );
    }

//@ts-ignore
    return <CommandButtonType {...commandButtonProps} defaultRender={CommandBarButton} />;
  };
